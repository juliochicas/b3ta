...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching meeting ${meetingId} for cancellation notification`);
    ...
    if (recipients.length === 0) {
      console.log("No recipients found for cancellation notification");
      return new Response(
    ...
    console.log(`Sending cancellation notification to ${finalRecipients.length} recipients`, finalRecipients.map(r => r.email));
    ...
    const formattedDate = meetingDate.toLocaleDateString("es-MX", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    const formattedTime = meetingDate.toLocaleTimeString("es-MX", {
      hour: "2-digit",
      minute: "2-digit",
          ...
          <html>
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="color: white; margin: 0; font-size: 28px;">Reunión Cancelada</h1>
              </div>
              ...
              <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
                ...
                <p style="font-size: 16px; margin-bottom: 20px;">
                  Lamentamos informarte que la siguiente reunión ha sido <strong>cancelada</strong>:
                </p>
                
                <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #e53e3e; margin-bottom: 20px;">
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `Notificación de cancelación enviada a ${finalRecipients.length} destinatario(s)` 
      }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: any) {
    console.error("Error in send-meeting-cancellation function:", error);
    return new Response(
