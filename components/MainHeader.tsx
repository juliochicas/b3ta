  ...
  return (
    <header className="fixed top-0 w-full bg-white/95 backdrop-blur-sm z-50 border-b border-gray-100">
      <div className="container mx-auto px-4 sm:px-6 py-4 flex items-center justify-between max-w-6xl">
        <div className="cursor-pointer flex items-center gap-1" onClick={() => pathname !== "/" ? router.push("/") : window.scrollTo({ top: 0, behavior: "smooth" })}>
        ...
        <div className="flex items-center gap-3">
          <Link href="/auth" className="text-sm text-slate-500 hover:text-slate-900 transition-colors hidden sm:flex items-center gap-1">
      ...
      {open && (
        <div className="md:hidden bg-white border-b border-gray-100 px-4 py-4 space-y-3">
          ...
          <Link href="/blog" onClick={() => setOpen(false)} className="block text-base text-slate-600 hover:text-slate-900 py-2">Blog</Link>
